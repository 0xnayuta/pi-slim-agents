...
export function buildAgentHelpText(): string {
  return [
    ...
    '   /agent returns a specialist delegation prompt.',
    '   It does NOT execute grep/read/bash or start a child agent.',
    '   Use the generated prompt to guide the main agent, or ask pi to',
    '   perform the search manually (see examples below).',
    '',
    'Examples:',
    ...
    '',
    'Two-step dogfood pattern (get prompt then execute):',
    '  Step 1: /agent explorer find where playback scheduling is implemented',
    '  Step 2: Ask pi: "Using the Explorer instructions above, actually search',
    '          the repository for playback scheduling. Use grep/read/bash and',
    '          return path:line evidence."',
    '',
    'Direct search (no /agent):',
    '  Search the repository for playback scheduling. Use grep/read/bash.',
    '  Return path:line evidence.',
    '',
